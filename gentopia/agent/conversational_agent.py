...
class ConvAgent(ConvBaseAgent):
    ...
    def clear_conv_history(self, recipient: ConvBaseAgent):
        """
        ...
        :type recipient: ConvAgent
        """
        if recipient.name in self._conv_history:
            self._conv_history[recipient.name].clear()
        else:
    ...
    @property
    def conversation_history(self, recipient: Optional[ConvBaseAgent] = None):
        if recipient:
